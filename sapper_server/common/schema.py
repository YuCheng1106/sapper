...
from pydantic import BaseModel, ConfigDict, EmailStr, validate_email, HttpUrl
...
class SchemaBase(BaseModel):
    ...
    def to_dict(self, exclude_none: bool = True) -> Dict[str, Any]:
        """
        将模型转换为字典，自动处理所有 datetime 字段

        :param exclude_none: 是否排除值为 None 的字段
        :return: 处理后的字典
        """
    ...
    def _convert_datetime_fields(data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for key, value in data.items():
            if isinstance(value, datetime):
                ...
                result[key] = SchemaBase._convert_datetime_fields(value)
            elif isinstance(value, list):
                result[key] = [
                    SchemaBase._convert_datetime_fields(item) if isinstance(item, dict) else

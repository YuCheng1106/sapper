...
from plugin.llm_provider.schema import CreateLlmProviderParam, UpdateLlmProviderParam
...
class CRUDLlmProvider(CRUDPlus[LlmProvider]):
    ...
    async def get(self, db: AsyncSession, pk: int) -> LlmProvider | None:
        ...
        :return:
        """
        return await self.select_model(db, pk, load_strategies=['models', 'provider_configs'])
    ...
    async def get_list(
        ...
        query = await self.select_order(
            ...
            'desc',
            load_options=[
                noload(self.model.models),
                selectinload(self.model.provider_configs)
            ]
        )
    ...
    async def get_all(self, db: AsyncSession) -> Sequence[LlmProvider]:
        """
        获取所有数据库
    ...
    async def update(self, db: AsyncSession, pk: int, obj: UpdateLlmProviderParam) -> int:
        """
        更新数据库

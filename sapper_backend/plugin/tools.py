...
from common.enums import DataBaseType, PrimaryKeyType, StatusType
from common.exception import errors
from common.log import log
from core.conf import settings
from core.path_conf import PLUGIN_DIR
from database.redis import RedisCli, redis_client
from utils._await import run_await
...
def get_plugin_models() -> list[type]:
    ...
    for plugin in get_plugins():
        # 导入插件的模型模块
        module_path = f'plugin.{plugin}.model'
...
def inject_extend_router(plugin: dict[str, Any]) -> None:
    ...
    for root, _, api_files in os.walk(plugin_api_path):
        for file in api_files:
            ...
            module_path = f'plugin.{path_to_module_str}'
            ...
            try:
                ...
                target_module_path = f'app.{app_name}.api.{relative_path.replace(os.sep, ".")}'
...
def inject_app_router(plugin: dict[str, Any], target_router: APIRouter) -> None:
    ...
    """
    plugin_name: str = plugin['plugin']['name']
    module_path = f'plugin.{plugin_name}.api.router'
    try:
        module = import_module_cached(module_path)

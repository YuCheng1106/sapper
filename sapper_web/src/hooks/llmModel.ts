...
export function useDispatchLLMModel() {
    ...
    const getLLMModelDetail = useCallback((pk: number) => {
        return dispatch(fetchLLMModelDetail(pk));
    }, [dispatch]);
    ...
    const updateLLMModelInfo = useCallback((pk : number, data: LLMModelUpdateReq) => {
        return dispatch(updateLLMModel({ pk, data }));
    }, [dispatch]);

    // Delete LLMModels
    const removeLLMModel = useCallback((pk: number) => {
